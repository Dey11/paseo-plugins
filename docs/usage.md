...
- Open **Workspace notes** from the Explorer **+** menu or the Command Center. Notes opens in the right-side Explorer beside Changes and Files. Edit Markdown in **Write**, inspect it in **Preview**, and save it to the daemon.
...
- Open **Temp chat** from the Explorer **+** menu or Command Center. It creates a separate agent for that workspace, so questions and answers never enter the primary agent conversation.
- Before the first message, choose the model, mode, and reasoning level. The first question captures the workspace note and a bounded set of recent user/assistant messages from other agents in the workspace. The chat keeps that context until you select **Refresh context**; the refreshed snapshot is attached to the next question without producing a separate refresh turn.
- Temp Chat is for read-only clarification. The panel renders only your questions and the answers, not the internal context envelope. While its agent is working, the composer pauses instead of queueing another question.
- Select **Archive**, then **Archive chat**, to soft-delete the dedicated agent and start fresh. The archived transcript remains available through Paseo's normal archive/history surfaces.
