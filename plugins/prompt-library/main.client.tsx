...
export function PromptLibrarySurface({ theme, layout }: PluginSurfaceProps) {
  ...
  const prompts = useQuery({
    queryKey: ["prompt-library", query],
    queryFn: () => listPrompts({ query }),
  });
  useEffect(() => {
    const timer = setTimeout(() => prompts.refetch(), 180);
  ...
  async function save() {
    ...
    try {
      ...
      setNotice(
        "Prompt saved. It is now available in the composer attachment picker.",
      );
    } catch (error) {
      ...
      <View style={styles.header}>
        <View style={styles.grow}>
          ...
          <Text style={styles.muted}>
            Save once, then attach a prompt from any workspace composer.
          </Text>
        </View>
        <Button label="New prompt" onPress={clear} styles={styles} />
      </View>
          ...
          <View style={styles.stack}>
            {prompts.data?.map((prompt) => (
              ...
              >
                ...
                {prompt.tags.length ? (
                  <Text style={styles.meta}>{prompt.tags.join(" · ")}</Text>
                ) : null}
              </Pressable>
            ))}
        ...
        <View style={styles.editor}>
          ...
          <View style={styles.actions}>
            ...
            {editingId ? (
              <Button
                label={
                  confirmDelete === editingId ? "Confirm delete" : "Delete"
                }
                onPress={() => remove(editingId)}
                disabled={busy}
                danger={confirmDelete === editingId}
                styles={styles}
              />
            ) : null}
          </View>
  ...
  return useMemo(
    () =>
      StyleSheet.create({
        ...
        content: { padding: compact ? 16 : 24, gap: 20 },
        header: {
          flexDirection: "row",
          gap: 12,
        ...
        listPane: { flex: 1, minWidth: 260, gap: 12 },
        editor: {
          flex: 1,
          minWidth: 260,
          gap: 10,
        ...
        contentInput: { minHeight: 220, textAlignVertical: "top" },
        card: {
          gap: 5,
          padding: 13,
          borderRadius: 12,
