...
export function searchPrompts(prompts: readonly SavedPrompt[], query: string): SavedPrompt[] {
...
export function upsertPrompt(
  ...
  if (input.id && !existing) throw new Error("The saved prompt no longer exists.");
  ...
  return { prompt, prompts: [prompt, ...prompts.filter((candidate) => candidate.id !== prompt.id)] };
}

export function deletePrompt(prompts: readonly SavedPrompt[], id: string): { deleted: boolean; prompts: SavedPrompt[] } {
